...
class nnUNetPredictor(object):
    def __init__(
        ...
        if device.type == "cuda":
            ...
            print(
                f"perform_everything_on_device=True is only supported for cuda devices! Setting this to False"
            )
    ...
    def manual_initialization(
        ...
        allow_compile = (
            allow_compile
            and ("nnUNet_compile" in os.environ.keys())
            and (os.environ["nnUNet_compile"].lower() in ("true", "1", "t"))
        )
        allow_compile = allow_compile and not isinstance(self.network, OptimizedModule)
        if isinstance(self.network, DistributedDataParallel):
            allow_compile = allow_compile and isinstance(
                self.network.module, OptimizedModule
            )
        if allow_compile:
            print("Using torch.compile")
            self.network = torch.compile(self.network)
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        for params in self.list_of_parameters:
            ...
            if prediction is None:
                prediction, enc_fea = self.predict_sliding_window_return_logits(data)
                prediction.to("cpu")
                enc_fea.to("cpu")
                print(f"prediction1: {prediction.shape}, {enc_fea.shape}")
            else:
                print('test, do we reach here 1?')
                prediction += self.predict_sliding_window_return_logits(data)[0].to("cpu")
                enc_fea += self.predict_sliding_window_return_logits(data)[1].to("cpu")

        if len(self.list_of_parameters) > 1:
            prediction /= len(self.list_of_parameters)
            enc_fea /= len(self.list_of_parameters)

        if self.verbose:
            print("Prediction done")
        torch.set_num_threads(n_threads)
        return prediction, enc_fea
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        prediction, enc_fea = self.network(x)
        #print(f"prediction 5: {prediction.shape}, {enc_fea.shape}")
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                sub_prediction, sub_enc_fea = self.network(torch.flip(x, axes))
                prediction += torch.flip(sub_prediction, axes)
                enc_fea += torch.flip(sub_enc_fea, axes)
            prediction /= len(axes_combinations) + 1
            enc_fea /= len(axes_combinations) + 1
        return prediction, enc_fea
        
    def compute_patch_counts(self, image_shape, patch_size, step_size):
        """
        Computes number of patches in each dimension (Z, Y, X) given the input size, patch size, and step.
    
        Args:
            image_shape (Tuple[int, int, int]): Shape of the input image [D, H, W]
            patch_size (Tuple[int, int, int]): Size of the sliding window patch [d, h, w]
            step_size (Tuple[int, int, int]): Step size in each direction [sd, sh, sw]
    
        Returns:
            Tuple[int, int, int]: Number of patches along (Z, Y, X)
        """
        patch_counts = tuple(
                    math.ceil((image_shape[i] - patch_size[i]) / step_size[i]) + 1
                    for i in range(3)
                )
        return patch_counts

    def assemble_encoder_features(self, patches, patch_shape, patch_counts, overlap=0.5, device=None):
        """
        Assembles encoder feature patches into a full feature map.
    
        Args:
            patches (List[Tensor]): List of encoder outputs of shape [C, D, H, W]
            patch_shape (Tuple[int, int, int]): The shape of a single encoder output patch [D, H, W]
            patch_counts (Tuple[int, int, int]): Number of patches along each axis [num_z, num_y, num_x]
            overlap (float): Amount of overlap between patches (0.5 = 50%)
    
        Returns:
            Tensor: Assembled encoder feature map of shape [C, D_total, H_total, W_total]
        """
        assert all(p.shape == (patches[0].shape) for p in patches), "Inconsistent patch shape"
        C, pd, ph, pw = patches[0].shape
        step_d, step_h, step_w = int(pd * (1 - overlap)), int(ph * (1 - overlap)), int(pw * (1 - overlap))
    
        total_d = step_d * (patch_counts[0] - 1) + pd
        total_h = step_h * (patch_counts[1] - 1) + ph
        total_w = step_w * (patch_counts[2] - 1) + pw
    
        # Allocate result tensors
        result = torch.zeros((C, total_d, total_h, total_w), dtype=patches[0].dtype, device=device)
        count_map = torch.zeros_like(result)
    
        idx = 0
        for iz in range(patch_counts[0]):
            for iy in range(patch_counts[1]):
                for ix in range(patch_counts[2]):
                    d = iz * step_d
                    h = iy * step_h
                    w = ix * step_w
                    patch = patches[idx]
                    result[:, d:d+pd, h:h+ph, w:w+pw] += patch
    ...
    def _internal_predict_sliding_window_return_logits(
        ...
        try:
            ...
            #patch_counts = self.compute_patch_counts(data.shape[1:], (56, 192, 192), (28, 96, 96))
            #print(patch_counts)
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                prediction, enc_fea = self._internal_maybe_mirror_and_predict(workon)
                prediction = prediction[0].to(
                    results_device
                )
                enc_fea = enc_fea[0].to(
                    results_device
                )
                
                #print(f"prediction4: {prediction.shape}, {enc_fea.shape}")
                ...
                patches.append(enc_fea)

            predicted_logits /= n_predictions
            patch_shape = (7, 24, 24)  # from encoder
            patch_counts = (1, 1, 1)
            enc_fea_full = self.assemble_encoder_features(patches, patch_shape, patch_counts, overlap=0.5, device=results_device)
            # check for infs
            ...
            raise e
        return predicted_logits, enc_fea_full
        ...
        with torch.no_grad():
            ...
            with (
                ...
                if self.perform_everything_on_device and self.device != "cpu":
                    ...
                    try:
                        predicted_logits, enc_fea = (
                            self._internal_predict_sliding_window_return_logits(
                                data, slicers, self.perform_everything_on_device
                            )
                        )
                        #print(f"predicted logits: {predicted_logits.shape}, {enc_fea.shape}")
                        # Log memory usage
                        allocated = torch.cuda.memory_allocated(self.device) / 1024**2  # in MB
                        reserved = torch.cuda.memory_reserved(self.device) / 1024**2    # in MB
                        print(f"[GPU {self.device}] Allocated: {allocated:.2f} MB, Reserved: {reserved:.2f} MB")
                ...
                #print(f"predicted logits revert padding: {predicted_logits.shape}, {enc_fea.shape}")
        return predicted_logits, enc_fea
    ...
    def predict_from_files_sequential(
        ...
        if output_folder is not None:
            ...
            for k in inspect.signature(
                self.predict_from_files_sequential
            ).parameters.keys():
                my_init_kwargs[k] = locals()[k]
            my_init_kwargs = deepcopy(
                my_init_kwargs
            ...
            if 'loaded_model' in my_init_kwargs:
              del my_init_kwargs['loaded_model']
        ...
        for li, of, sps in zip(
            ...
            prediction, enc_fea = self.predict_logits_from_preprocessed_data(
            ...
            #final_scalar = x.mean().item()
            #array = np.zeros((1, 1, 1), dtype=np.float32)
            #array[0, 0, 0] = final_scalar
            array = enc_fea.numpy().astype(np.float32)
            print(f'enc_fea1 shape: {array.shape}')
            pooled = array.mean(axis=(1, 2, 3))  # ? (256,)
            print(pooled.shape)
            pooled_reshape = pooled.reshape(1, -1)
            

            y_pred = loaded_model.predict(pooled_reshape)
            y_proba = loaded_model.predict_proba(pooled_reshape)[:, 1]
            print(y_pred, y_proba)
            
            patient_id = of.removeprefix(output_folder_or_list_of_truncated_output_files)
            #task2_path = os.path.join(task2_of, f"{patient_id}.npy")
            
            #np.save(task2_path, array)
            # Load existing data (if file exists), else start with empty dict
            # Check if file exists
            if os.path.exists(task2_of):
                # Load existing data
                with open(task2_of, 'r') as f:
                    data = json.load(f)
            else:
                # Start with empty dict if file doesn't exist
                data = {}
            
            # Add or update a key with two values
            # For example, key: "my_key" with two values in a list
            data[patient_id] = [y_pred.tolist(), y_proba.tolist()]  # Replace value1, value2 with your actual values
            
            # Or if you want to store as dictionary:
            # data['my_key'] = {"val1": value1, "val2": value2}
            
            # Write back to JSON file
            with open(task2_of, 'w') as f:
                json.dump(data, f, indent=4)
            
            
            #print(f"prediction: {prediction.shape}, {enc_fea.shape}")
...
if __name__ == "__main__":
    ...
    ret = predictor.predict_from_files_sequential(
        [
            [
                "/media/isensee/raw_data/nnUNet_raw/Dataset004_Hippocampus/imagesTs/hippocampus_002_0000.nii.gz"
            ],
            [
                "/media/isensee/raw_data/nnUNet_raw/Dataset004_Hippocampus/imagesTs/hippocampus_005_0000.nii.gz"
            ],
        ],
        "/home/isensee/temp/tmp",
        False,
        True,
        None,
    )
