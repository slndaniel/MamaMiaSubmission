...
from monai.networks.blocks.segresnet_block import ResBlock, get_conv_layer, get_upsample_layer, ChannelAttention3D, SpatialAttention3D, CBAM3D, AttentionResBlock3D, AttentionLightweightResBlock3D, LightweightResBlock3D
...
class SegResNetFR(nn.Module):
    ...
    def _make_down_layers(self):
        ...
        for i, item in enumerate(blocks_down):
            ...
            down_layer = nn.Sequential(
                pre_conv, *[LightweightResBlock3D(spatial_dims, layer_in_channels, norm=norm, act=self.act) for _ in range(item)]
            )
    ...
    def _make_up_layers(self):
        ...
        for i in range(n_up):
            ...
            up_layers.append(
                nn.Sequential(
                    *[
                        LightweightResBlock3D(spatial_dims, sample_in_channels // 2, norm=norm, act=self.act)
              ...
              up_samples.append(
                nn.Sequential(
                    *[
                        ...
                        get_upsample_layer(spatial_dims, sample_in_channels // 2, upsample_mode=upsample_mode, size=(7, 24, 24), scale_use=True),
                    ]
                )
            )
            else:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x, seg = self.decode(x, down_x)
        
        if self.predict:
          if self.out_encoder:
            return seg, encode_feature
          else:
            return seg
        else:
          return x, seg
